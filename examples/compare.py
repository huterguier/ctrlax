...
def main():
    ...
    # --- Position vs. goal ---
    ax_pos.axhline(GOAL, color=INK_MUTED, linewidth=1.5, linestyle=(0, (4, 3)), zorder=1)
    ax_pos.text(NUM_STEPS - 1, GOAL, "  goal", color=INK_MUTED, fontsize=9, va="center")
    ax_pos.plot(t, rs_pos, color=COLOR_RANDOM_SHOOTING, linewidth=2, label="RandomShooting", zorder=3)
    ax_pos.plot(t, cem_pos, color=COLOR_CEM, linewidth=2, label="CEM", zorder=3)
    ax_pos.set_title("Position vs. goal", color=INK_PRIMARY, fontsize=11, loc="left", weight="bold")
    ...
    ax_cost.plot(t, rs_cost, color=COLOR_RANDOM_SHOOTING, linewidth=2, label="RandomShooting", zorder=3)
    ax_cost.plot(t, cem_cost, color=COLOR_CEM, linewidth=2, label="CEM", zorder=3)
    ax_cost.set_yscale("log")
    ax_cost.set_title("Best planning cost per step", color=INK_PRIMARY, fontsize=11, loc="left", weight="bold")
    ...
    fig.legend(
        ...
        bbox_to_anchor=(0.5, -0.03), labelcolor=INK_PRIMARY, fontsize=10,
    )
    fig.suptitle(
        "RandomShooting vs. CEM — matched compute budget (1000 rollouts/step)",
        color=INK_PRIMARY, fontsize=12, weight="bold", y=1.02,
    )
    ...
    print(f"{'final |pos-goal|':14s} {abs(rs_pos[-1] - GOAL):16.4f} {abs(cem_pos[-1] - GOAL):10.4f}")
    print(f"{'final best_cost':14s} {rs_cost[-1]:16.4f} {cem_cost[-1]:10.4f}")
    print(f"{'mean best_cost':14s} {sum(rs_cost) / len(rs_cost):16.4f} {sum(cem_cost) / len(cem_cost):10.4f}")
